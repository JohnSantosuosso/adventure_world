...
import { selectCharacter } from './welcome';
...
async function outskirtsCity(): Promise<void> {
  const newCharacter = await selectCharacter();
  if (newCharacter) {
    await delay(5000);
...
async function oldWizardApproaches(newCharacter: Character) : Promise<void>{
  if (newCharacter) {
...
async function oldWizardQuestions(newCharacter: Character) : Promise<void>{
  if (newCharacter) {
    console.clear();
...
async function acceptQuest(newCharacter: Character): Promise<void> {
  if (newCharacter) {
    return inquirer
      .prompt({
        type: 'list',
      ...
      .then(answer => {
        switch (answer.option) {
          case 'Answer Riddle':
            ...
            delay(5000);
            console.clear();
            let currentRiddle = Riddle.getRandomRiddle();
